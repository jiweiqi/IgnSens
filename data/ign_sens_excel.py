...
def ign_uq(g, factor, temp, pres, fuel, phi, T_equi, simtype='UV', bootplot=False):
	...
	if 0:
		ref_pos = np.argmin( np.abs(temp - 966) )
		ign_pos = np.argmax( diff_temp[0:ref_pos] )
	else:
...
def ign_sens(g, temp, pres, fuel, phi, ign0, simtype='UV', bootplot=False, sensspecies='H2O2'):
	'''
	...
	# stateArray = []
	# sensspecies = 'CO'
	print(2+gas.species_index(sensspecies))
	while sim.time < ign0*1.5:
		sim.step()
		# print( sim.sensitivities().shape )
		sens_Y.append( sim.sensitivities()[2+gas.species_index(sensspecies)+1,:] )
		...
		time.append( sim.time )
		temp.append( r.T )
		# OH.append( r.thermo['OH'].Y )
		heatrelease.append( np.dot(gas.net_production_rates, gas.partial_molar_enthalpies)/gas.cv_mole )

	time = np.array(time)
	temp = np.array(temp)
	diff_temp = np.diff(temp)/np.diff(time)
	OH = np.array(OH)
	heatrelease = np.array(heatrelease)
	...
	np.savetxt('senstime.out', (time, temp))
	np.savetxt('YOH.out', (OH))
	np.savetxt('heatrelease.out', (heatrelease))
	np.savetxt('sens.out', (sens_T))
	np.savetxt('sens'+sensspecies, (sens_Y))
	...
	for temp in [750]:
		for phi in [1.0]:
			for simtype in ['UV']:
				...
				print('Start Adjoint')
				for sensspecies in ['CO', 'CO2', 'H2O', 'OH', 'H', 'O','H2O2']:
				# for sensspecies in ['H2O']:
				#for sensspecies in ['CH3OCH3','CH2O' ,'CO2', 'CO' ,'H2O', 'OH', 'H', 'O', 'H2O2']:
					sens_T_ign = ign_sens(gas, temp, pres, fuel, phi, ign0, simtype, False, sensspecies)
				...
				np.savetxt('idtFD.txt', (ds['bruteforce']))